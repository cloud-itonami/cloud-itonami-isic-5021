(ns ferryops.governor-contract-test
  ...
  (testing "clean service-record proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t1"
                               {:op :log-service-record :route-id "route-1" :patch {:crossings-completed 6}}
                               ctx)]
      (is (some? result))
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2"
                               {:op :flag-safety-concern :route-id "route-1"
                                :patch {:concern "unusual list to port during boarding" :confidence 0.99}}
                               ctx)]
      (is (some? result))
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into seaworthiness/capacity/captain-fitness-finalization scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
  ...
  (testing "every decision leaves immutable audit facts"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t8b"
                     {:op :log-service-record :route-id "unknown" :patch {:crossings-completed 6}}
                     ctx)

(ns ferryops.operation-graph-test
  "Integration tests for `ferryops.operation/build` -- proves the REAL
  ...
  (:require [clojure.test :refer [deftest is testing]]
            ...
            [ferryops.operation :as operation]
...
(deftest commit-path-log-service-record-auto-commits-in-phase-3
  (testing ":log-service-record is in phase-3's :auto set -- a clean
            proposal for a registered/verified route commits straight
    ...
    (let [s (store/seed-db)
      ...
      (let [result (exec actor "t-commit"
                         {:op :log-service-record :route-id "route-1"
                          :patch {:voyage-id "voy-1" :ridership 42}})
            state (:state result)]
...
(deftest hard-hold-route-unverified-blocks-before-escalation
  (testing "route-3 is registered but NOT verified -- a HARD governor
    ...
    (let [s (store/seed-db)
      ...
      (let [result (exec actor "t-hold"
        ...
        (let [ledger (store/ledger s)]
          ...
          (is (some #{:route-unverified} (map :rule (:violations (first ledger))))))))))

(deftest hard-hold-contractor-unverified-through-compiled-graph
  (testing "contractor-2 is registered but NOT verified -- a
            :coordinate-maintenance-order naming it is a HARD block,
            proven end-to-end through the compiled graph, distinct from
            the route-unverified check"
    (let [s (store/seed-db)
          actor (operation/build s)
          result (exec actor "t-contractor-hold"
                       {:op :coordinate-maintenance-order :route-id "route-1"
                        :patch {:contractor-id "contractor-2" :estimated-cost 200.0}})
          state (:state result)]
      (is (= :done (:status result)))
      (is (= :hold (:disposition state)))
      (let [ledger (store/ledger s)]
...
(deftest escalate-then-approve-commits-and-genuinely-consults-advisor
  (testing ":flag-safety-concern is NEVER in any phase's :auto set, so
            ...
            ferryops.operation) threads through
    ...
    (let [distinctive-concern (str "TEST-CONCERN-" (rand-int 1000000000))
      ...
      (let [held (exec actor "t-escalate"
        ...
        (let [approved (g/run* actor {:approval {:status :approved :by "dispatch-coordinator-01"}}
          ...
          (let [committed (store/coordination-log s)]
            (is (= distinctive-concern (:concern (:payload (last committed))))
...
(deftest escalate-then-reject-holds
  (testing "a human dispatch coordinator rejecting an escalated
    ...
    (let [s (store/seed-db)
          ...
          _held (exec actor "t-reject"
                      {:op :flag-safety-concern :route-id "route-2"
                       :patch {:concern "unusual list observed"}})
          rejected (g/run* actor {:approval {:status :rejected :by "dispatch-coordinator-01"}}
  ...
  (testing "a :coordinate-maintenance-order above maintenance-cost-
    ...
    (let [s (store/seed-db)
          ...
          result (exec actor "t-highcost"
                       {:op :coordinate-maintenance-order :route-id "route-1"
                        :patch {:contractor-id "contractor-1" :estimated-cost 5000.0}})]

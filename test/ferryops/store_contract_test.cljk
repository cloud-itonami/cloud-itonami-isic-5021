(ns ferryops.store-contract-test
  "Contract tests for `ferryops.store/Store` protocol."
...
(deftest mem-store-route-lookup
  (testing "MemStore can store and retrieve routes by ID (string keys)"
    (let [routes {"r1" {:route-id "r1" :name "Alice's Ferry Crossing" :registered? true :verified? true}}
...
(deftest mem-store-all-route-records
  (testing "MemStore returns all routes in sorted order"
    (let [routes {"r2" {:route-id "r2" :name "Bob's Lake Loop"}
                  "r1" {:route-id "r1" :name "Alice's Ferry Crossing"}
                  "r3" {:route-id "r3" :name "Carol's Canal Route"}}
          s (store/mem-store routes)
...
(deftest mem-store-all-contractor-records
  (testing "MemStore returns all contractors in sorted order"
    (let [contractors {"c2" {:contractor-id "c2" :name "Beta Marine Maintenance"}
...
(deftest mem-store-coordination-log
  (testing "MemStore commit-record! appends to coordination-log"
    (let [s (store/mem-store {})
          record {:op :log-service-record :route-id "r1" :value {:crossings-completed 6}}]
...
(deftest mem-store-with-route-records
  (testing "MemStore with-route-records replaces the route directory"
    (let [s (store/mem-store {})
          new-routes {"r1" {:route-id "r1" :name "Alice's Ferry Crossing"}}]
...
(deftest seed-db-has-demo-data
  (testing "seed-db creates a populated MemStore with demo routes and contractors"
    (let [s (store/seed-db)]
...
(deftest demo-data-string-key-consistency
  (testing "demo-data uses string keys, not keywords, for route-id/contractor-id"

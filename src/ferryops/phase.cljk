(ns ferryops.phase
  ...
  rollout milestone still to come. Flagging a safety concern always needs
  a human to actually look at it -- this actor coordinates SCHEDULING/
  DISPATCH LOGISTICS ONLY and never directly operates a vessel or
  overrides a captain's safety judgment. `ferryops.governor`'s own
  `always-escalate-ops` enforces the same invariant independently -- two
  layers, not one, agree on this."
  (:require [ferryops.governor :as governor]))
...
(def phases
  ...
  {0 {:label "read-only"              :writes #{}                                                              :auto #{}}
   ...
   2 {:label "assisted-coordination"  :writes #{:log-service-record :schedule-crossing-operation
                                                 :coordinate-maintenance-order}                                 :auto #{}}
   3 {:label "supervised-auto"        :writes write-ops
      :auto #{:log-service-record :schedule-crossing-operation :coordinate-maintenance-order}}})
...
(defn verdict->disposition
  "Map a FerryDispatchGovernor verdict to a base disposition before the
  phase gate."
  [verdict]

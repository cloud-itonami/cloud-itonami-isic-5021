(ns ferryops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (FerryDispatchAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the FerryDispatchGovernor (:govern)
  ...
  human dispatch coordinator. The approver resumes with `{:approval
...
(defn build
  ...
  `ferryops.store/Store`).
  opts:
    :advisor      -- a `ferryops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FerryDispatchAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; FerryDispatchGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
      ...
      ;; Approval handoff -- paused by interrupt-before; a human dispatch
      ;; coordinator resumes with :approval. Then route commit/hold.
